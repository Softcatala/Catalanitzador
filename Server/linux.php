<?php $title = "Catalanitzador de Softcatalà - Com configurar el Linux en català"; ?>
<?php $description = "Catalanitzador de Softcatalà: explicació pas a pas de com configurar el Linux en català"; ?>
<?php include('header.php'); ?>

<div class="center">
    <div class="container">
    <span class="breadcrumbs"><a href="/" title="Inici">Inici</a> » Com configurar el Linux en català</span>
    <h1 class="page">Catalanitzador <span class="so">de Softcatalà</span></h1>
    <h2>Com configurar el Linux en català</h2>
    	<div class="download_section general">

<p>Establir el català com a llengua per defecte d'un sistema operatiu Linux depèn de la distribució 
i de l'entorn d'escriptori que es faci servir.
</p>

<h3>Selecció de llengua durant la instal·lació</h3>
<ul class="general">
    <li>Podeu baixar distribucions des del rebost de Softcatalà:<br/>
        - <a href="http://www.softcatala.org/wiki/Rebost:Ubuntu" title="Fitxa de l'Ubuntu al rebost de Softcatalà">Baixada de l'Ubuntu</a>
        <br/>- <a href="http://www.softcatala.org/wiki/Rebost:Fedora" title="Fitxa de la Fedora al rebost de Softcatalà">Baixada de la Fedora</a>
        <br/>- <a href="http://www.softcatala.org/wiki/Rebost:OpenSUSE" title="Fitxa de l'OpenSUSE al rebost de Softcatalà">Baixada de l'OpenSUSE</a>
        <br/>- <a href="http://www.softcatala.org/wiki/Rebost:Linux_Mint" title="Fitxa del Linux Mint al rebost de Softcatalà">Baixada del Linux Mint</a>
    </li>
    <li>Quan s'iniciï la instal·lació, seleccioneu el català en el segon pas</li>
    <li>A partir d'aquí tot el sistema es configurarà en català</li>
</ul>
<br class="p"/>
<h3>Selecció de llengua a l'escriptori</h3>
<ul class="general">
    <li>Segons l'entorn d'escriptori Linux que feu servir, el procediment de selecció de llengua és diferent</li>
    <li>Heu d'accedir a les preferències generals del sistema i cercar les seccions de «Locale», «Idioma» o «Llengua» i 
    des d'allà seleccionar el català</li>
</ul>

<br class="p"/>
<br class="p"/>

<p>Si teniu cap dubte o us cal ajuda per configurar el vostre mòbil o tauleta podeu adreçar-vos al <a href="http://www.softcatala.org/forum/viewforum.php?f=3&sid=04e08382c279bf4d89b5add56a481378">fòrum de Linux</a>.</p>

<br class="p"/>

</div>


       <div class="so_screenshot_section">
            <ul id="slider_android">
                <li><img src="/images/so/linux1.png" alt="Catalanitzar el Linux captura 1"/></li>
                <li><img src="/images/so/linux2.png" alt="Catalanitzar el Linux captura 2"/></li>
                <li><img src="/images/so/linux3.png" alt="Catalanitzar el Linux captura 3"/></li>
                <li><img src="/images/so/linux4.png" alt="Catalanitzar el Linux captura 4"/></li>
            </ul>
            
        </div>

	</div>
</div>

<?php include('footer.html'); ?>
